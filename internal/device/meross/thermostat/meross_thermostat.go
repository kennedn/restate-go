// Package thermostat provides control logic for Meross thermostat devices.
package thermostat
...
// Routes returns thermostat routes for the given configuration.
...
func routes(config *config.Config, internalConfigPath string) (*base, []router.Route, error) {
	...
	if internalConfigPath == "" {
		internalConfigPath = "./internal/device/meross/thermostat/device.yaml"
	}
